...
def plot_model_results(df):
    ...
    for index, row in df.iterrows():
        ...
        labels.append("Iteration {}".format(labels_idx))
        labels_idx = labels_idx + 1
        reward_steps = []

    iterations_array = np.arange(start=1, stop=total_episodes + 1)

    plot_graph(iterations_array, steps_array_list, colors, labels, 'Steps taken in each episode',
               'Episode', 'Mean steps taken')
    plot_graph(iterations_array, reward_steps_list, colors, labels, 'Final reward in each episode',
               'Episode', 'Reward')
    plot_boxplot(steps_array_list, "Steps taken", "Iteration", "Steps taken in each episode", colors)
